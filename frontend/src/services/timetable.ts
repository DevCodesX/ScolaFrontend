...
const API = apiUrl("/api/timetable");
...
export async function getTeacherTimetable(): Promise<TimetableSlot[]> {
    ...
    if (!res.ok) throw new Error(`Failed to fetch timetable [${res.status} ${res.statusText}]`);
    return res.json();
}
...
export async function addSlot(data: {
    ...
    if (!res.ok) throw new Error(`Failed to add slot [${res.status} ${res.statusText}]`);
}
...
export async function deleteSlot(id: string): Promise<void> {
    ...
    if (!res.ok) throw new Error(`Failed to delete slot [${res.status} ${res.statusText}]`);
}
