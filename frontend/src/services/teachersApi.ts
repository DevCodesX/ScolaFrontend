...
export async function fetchTeachers(institutionId: string): Promise<Teacher[]> {
    ...
    if (!res.ok) throw new Error(`Failed to fetch teachers [${res.status} ${res.statusText}]`);
...
export async function createTeacher(data: any): Promise<Teacher> {
    ...
    if (!res.ok) throw new Error(`Failed to add teacher [${res.status} ${res.statusText}]`);
...
export async function updateTeacher(id: string, data: Partial<TeacherPayload>): Promise<Teacher> {
    ...
    if (!res.ok) throw new Error(`Failed to update teacher [${res.status} ${res.statusText}]`);
...
export async function deleteTeacher(id: string): Promise<void> {
    ...
    if (!res.ok) throw new Error(`Failed to delete teacher [${res.status} ${res.statusText}]`);
}
