...
const API_URL = apiUrl("/api/attendance");
...
export async function markAttendance(
    ...
    if (!res.ok) throw new Error(`Failed to mark attendance [${res.status} ${res.statusText}]`);
}
...
export async function markBulkAttendance(
    ...
    if (!res.ok) throw new Error(`Failed to save attendance [${res.status} ${res.statusText}]`);
}
